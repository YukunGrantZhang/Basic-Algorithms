/*
 * BreadthFirstSearchShortestPath.c
 *
...
void BreadthFirstSearch(struct Graph* graph, int starting_vertex, int n)
{
	...
	while (!queue_isEmpty(Q))
	{
		...
		while (temp != NULL)
		{
			...
			{
				graph->head[temp->dest]->dist = graph->head[pop_vertex]->dist + 1;
				graph->head[temp->dest]->explored = 1;
				insert_queue(Q, temp->dest);

				printf("Vertex: %d Distance: %d\n", temp->dest, graph->head[temp->dest]->dist);
			}
