/*
 * MSTKruskal'sPathCompressionAlgorithm.c
 *
...
void DepthFirstSearch(struct Graph* graph, int vertex, int leader)
{
	...
	struct Node* parent_node = graph->head[leader];

	if (parent_node->leader != NULL)
	{
	...
	graph->head[vertex]->leader = parent_node;
...
{
	int previous_rank = input_node->rank;
	int new_rank = 0;
...
void InsertEdge(struct Graph* graph, struct Edge edge)
{
	...
	if (graph->head[edge.src] != NULL && graph->head[edge.dest] != NULL)
	{
		...
		{
			struct Node* temp_node = graph->head[edge.src]->leader;

			while(temp_node->leader != temp_node)
			{
				temp_node = temp_node->leader;
			}

			leader_Node = temp_node;

			graph->head[edge.dest]->leader->leader = leader_Node;
		}
		else
		{
			struct Node* temp_node = graph->head[edge.dest]->leader;
